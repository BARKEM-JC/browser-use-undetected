...
def require_initialization(func):
    ...
    assert asyncio.iscoroutinefunction(
        func), '@require_initialization only supports decorating async methods on BrowserSession'
...
class BrowserSession(BaseModel):
    ...
    async def setup_browser_via_passed_objects(self) -> None:
        ...
        self.browser_context = (
                                           self.agent_current_page and self.agent_current_page.context) or self.browser_context or None
    ...
    async def setup_browser_via_browser_pid(self) -> None:
        ...
        debug_port = next((arg for arg in args if arg.startswith('--remote-debugging-port=')), '').split('=')[
            -1].strip()
        assert debug_port, (
    ...
    async def setup_new_browser_context(self) -> None:
        ...
        try:
            ...
            new_chrome_procs = [proc for proc in new_child_procs if
    ...
    async def _setup_current_page_change_listeners(self) -> None:
        ...
        try:
            await self.browser_context.expose_binding('_BrowserUseonTabVisibilityChange',
            ...
            async def perform_click(click_func):
                ...
                if self.browser_profile.downloads_dir:
                    try:
                        ...
                        unique_filename = await self._get_unique_filename(self.browser_profile.downloads_dir,
                                                                          suggested_filename)
    ...
    async def find_file_upload_element_by_index(self, index: int) -> DOMElementNode | None:
        ...
        try:
            ...
            def is_file_input(node: DOMElementNode) -> bool:
                return isinstance(node, DOMElementNode) and node.tag_name == 'input' and node.attributes.get(
                    'type') == 'file'
